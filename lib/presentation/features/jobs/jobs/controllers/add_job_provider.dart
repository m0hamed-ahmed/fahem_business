...
class AddJobProvider with ChangeNotifier {
  ...
  bool isAllDataValid(BuildContext context) {
    ...
    else if(_features.isEmpty) {return false;}
    return true;
  }
  ...
  Future<void> insertJob({required BuildContext context, required JobModel jobModel}) async {
    ...
    response.fold((failure) async {
      ...
      InsertJobParameters parameters = InsertJobParameters(
        jobModel: jobModel,
      );
      Either<Failure, JobModel> response = await jobsProvider.insertJobImpl(parameters);
      response.fold((failure) async {
        changeIsLoading(false);
        Dialogs.failureOccurred(context, failure);
      }, (job) {
        ...
        Dialogs.showBottomSheetMessage(
          context: context,
          message: Methods.getText(StringsManager.addedSuccessfully, appProvider.isEnglish).toTitleCase(),

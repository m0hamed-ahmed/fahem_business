...
import 'package:fahem_business/presentation/features/authentication/pages/lawyer_account_data_page.dart';
import 'package:fahem_business/presentation/features/authentication/pages/lawyer_personal_data_page.dart';
...
class RegisterLawyerProvider with ChangeNotifier {
  ...
  final List<Widget> _pages = const [
    LawyerPersonalDataPage(),
    LawyerAccountDataPage(),
  ];
  ...
  Future<void> insertLawyer({required BuildContext context}) async {
    LawyerProvider lawyerProvider = Provider.of<LawyerProvider>(context, listen: false);

    changeIsLoading(true);

    // Is All Lawyer Data Valid
    IsAllLawyerDataValidParameters parameters = IsAllLawyerDataValidParameters(
      emailAddress: _emailAddress!,
      password: _password!,
      phoneNumber: _phoneNumber!,
    );
    Either<Failure, bool> response = await lawyerProvider.isAllLawyerDataValidImpl(parameters);
    response.fold((failure) async {
      ...
      response.fold((failure) async {
        ...
        LawyerModel lawyerModel = LawyerModel(
          ...
          isVerified: false,
          isSubscriberToInstantLawyerService: false,
          isSubscriberToInstantConsultationService: false,
          isSubscriberToSecretConsultationService: false,
          isSubscriberToEstablishingCompaniesService: false,
          isSubscriberToRealEstateLegalAdviceService: false,
          isSubscriberToInvestmentLegalAdviceService: false,
          isSubscriberToTrademarkRegistrationAndIntellectualProtectionServ: false,
          isSubscriberToDebtCollectionService: false,
          rating: 0,
          createdAt: DateTime.now(),
        );
        if(_images.isEmpty) {await insertLawyerWithoutImages(context: context, lawyerModel: lawyerModel);}
        else {await insertLawyerWithImages(context: context, lawyerModel: lawyerModel);}
      });
    });
  }

  Future<void> insertLawyerWithoutImages({required BuildContext context, required LawyerModel lawyerModel}) async {
    LawyerProvider lawyerProvider = Provider.of<LawyerProvider>(context, listen: false);

    // Insert Lawyer
    InsertLawyerParameters parameters = InsertLawyerParameters(
      lawyerModel: lawyerModel,
    );
    Either<Failure, LawyerModel> response = await lawyerProvider.insertLawyerImpl(parameters);
    response.fold((failure) async {
      changeIsLoading(false);
      Dialogs.failureOccurred(context, failure);
    }, (lawyer) async {
      await NotificationService.subscribeToTopic(FirebaseConstants.fahemBusinessTopic).then((value) async {
        await NotificationService.subscribeToTopic('${lawyer.lawyerId}${ConstantsManager.fahemBusinessLawyersKeyword}').then((value) {
          ...
          CacheHelper.setData(key: PREFERENCES_KEY_ACCOUNT_TYPE, value: AccountTypeEnum.lawyers.name);
  ...
  Future<void> insertLawyerWithImages({required BuildContext context, required LawyerModel lawyerModel}) async {
    UploadFileProvider uploadFileProvider = Provider.of<UploadFileProvider>(context, listen: false);
    LawyerProvider lawyerProvider = Provider.of<LawyerProvider>(context, listen: false);

...
import 'package:fahem_business/presentation/features/lawyer/lawyer_profile/pages/lawyer_profile_account_data_page.dart';
import 'package:fahem_business/presentation/features/lawyer/lawyer_profile/pages/lawyer_profile_personal_data_page.dart';
import 'package:flutter/material.dart';
import 'package:geolocator/geolocator.dart';
import 'package:image_picker/image_picker.dart';
import 'package:provider/provider.dart';

class LawyerProfileProvider with ChangeNotifier {
  final EditLawyerUseCase _editLawyerUseCase;

  LawyerProfileProvider(this._editLawyerUseCase);

  Future<Either<Failure, LawyerModel>> editLawyerImpl(EditLawyerParameters parameters) async {
    return await _editLawyerUseCase.call(parameters);
  }
  ...
  final List<Widget> _pages = const [
    LawyerProfilePersonalDataPage(),
    LawyerProfileAccountDataPage(),
  ];
  ...
  Future<void> editLawyer({required BuildContext context}) async {
    ...
    LawyerModel lawyerModel = LawyerModel(
      lawyerId: accountModel.lawyerId,
      name: _name!,
      ...
      isSubscriberToInstantLawyerService: accountModel.isSubscriberToInstantLawyerService,
      isSubscriberToInstantConsultationService: accountModel.isSubscriberToInstantConsultationService,
      isSubscriberToSecretConsultationService: accountModel.isSubscriberToSecretConsultationService,
      isSubscriberToEstablishingCompaniesService: accountModel.isSubscriberToEstablishingCompaniesService,
      isSubscriberToRealEstateLegalAdviceService: accountModel.isSubscriberToRealEstateLegalAdviceService,
      isSubscriberToInvestmentLegalAdviceService: accountModel.isSubscriberToInvestmentLegalAdviceService,
      isSubscriberToTrademarkRegistrationAndIntellectualProtectionServ: accountModel.isSubscriberToTrademarkRegistrationAndIntellectualProtectionServ,
      isSubscriberToDebtCollectionService: accountModel.isSubscriberToDebtCollectionService,
    ...
    if(_image == null && _images.isEmpty) {
      debugPrint('_editLawyerWithoutImageAndWithoutImages');
      _editLawyerWithoutImageAndWithoutImages(context, lawyerModel);
    }
    else if(_image != null && _images.isNotEmpty) {
      debugPrint('_editLawyerWithImageAndWithImages');
      _editLawyerWithImageAndWithImages(context, lawyerModel);
    }
    else if(_image == null && _images.isNotEmpty) {
      debugPrint('_editLawyerWithoutImageAndWithImages');
      _editLawyerWithoutImageAndWithImages(context, lawyerModel);
    }
    else if(_image != null && _images.isEmpty) {
      debugPrint('_editLawyerWithImageAndWithoutImages');
      _editLawyerWithImageAndWithoutImages(context, lawyerModel);
    }
  }

  Future<void> _editLawyerWithoutImageAndWithoutImages(BuildContext context, LawyerModel lawyerModel) async {
    AppProvider appProvider = Provider.of<AppProvider>(context, listen: false);
    ...
    // Edit Lawyer
    EditLawyerParameters parameters = EditLawyerParameters(
      lawyerModel: lawyerModel,
    );
    Either<Failure, LawyerModel> response = await editLawyerImpl(parameters);
  ...
  Future<void> _editLawyerWithImageAndWithImages(BuildContext context, LawyerModel lawyerModel) async {
    AppProvider appProvider = Provider.of<AppProvider>(context, listen: false);
    ...
    UploadFileParameters uploadFileParameters = UploadFileParameters(
      ...
      directory: ApiConstants.lawyersDirectory,
    );
    ...
    response.fold((failure) async {
      ...
      lawyerModel.personalImage = image;

      for(int i=0; i<_images.length; i++) {
        // Upload Image
        UploadFileParameters uploadFileParameters = UploadFileParameters(
          file: File(_images[i].path),
          directory: ApiConstants.lawyersGalleryDirectory,
        );
        Either<Failure, String> response = await uploadFileProvider.uploadFileImpl(uploadFileParameters);
        response.fold((failure) async {
          changeIsLoading(false);
      ...
      response.fold((failure) async {
        ...
        changeImage(null);
        imagesNamesFromDatabase = lawyerModel.images;
        changeImages([]);
        Dialogs.showBottomSheetMessage(context: context, message: Methods.getText(StringsManager.modifiedSuccessfully, appProvider.isEnglish).toTitleCase(), showMessage: ShowMessage.success);
      });
    });
  ...
  Future<void> _editLawyerWithoutImageAndWithImages(BuildContext context, LawyerModel lawyerModel) async {
    AppProvider appProvider = Provider.of<AppProvider>(context, listen: false);
    UploadFileProvider uploadFileProvider = Provider.of<UploadFileProvider>(context, listen: false);
    LawyerProvider lawyerProvider = Provider.of<LawyerProvider>(context, listen: false);
  ...
  Future<void> _editLawyerWithImageAndWithoutImages(BuildContext context, LawyerModel lawyerModel) async {
    AppProvider appProvider = Provider.of<AppProvider>(context, listen: false);
    UploadFileProvider uploadFileProvider = Provider.of<UploadFileProvider>(context, listen: false);
    LawyerProvider lawyerProvider = Provider.of<LawyerProvider>(context, listen: false);
